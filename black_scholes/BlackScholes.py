...
class BlackScholes:
    def __init__(
        ...
        if time_to_exp_days < 7:
            ...
            self.time_list_display = [
                time_to_exp_days - (n * (np.ceil(time_to_exp_days / 7)))
                for n in range(0, 7)
            ]
        ...
        self.price_range_display = [
            self.current_underlying_price + self.current_underlying_price * (p / 100)
            for p in range(positive_bound, -1 * negative_bound - 1, -1 * step_size)
        ]
        self.perc_price_range_display = [
            p for p in range(-1 * negative_bound, positive_bound, step_size)
        ]
    ...
    def calculate_price(self) -> Tuple[List[List[int]], List[List[int]]]:
        ...
        for underlying_price in self.price_range_display:
            ...
            for dte in self.time_list_display:
                d1 = (1 / (self.volatility * np.sqrt(dte / 365))) * (
                    np.log(underlying_price / self.strike_price)
                    + (self.risk_free_rate + (self.volatility * self.volatility) / 2)
                    * (dte / 365)
                )
                d2 = d1 - self.volatility * np.sqrt(dte / 365)
                # Call
                call_price = norm.cdf(d1, 0, 1) * underlying_price - norm.cdf(
                    d2, 0, 1
                ) * self.strike_price * np.exp(-1 * self.risk_free_rate * (dte / 365))
                call_row.append(round(call_price, 3))
                # Put
                put_price = (
                    norm.cdf(-d2, 0, 1)
                    * self.strike_price
                    * np.exp(-self.risk_free_rate * dte / 365)
                    - norm.cdf(-d1, 0, 1) * underlying_price
                )
                ...
                for underlying_price in self.price_range_display:
                    ...
                    for dte in self.time_list_display:
                        d1 = (1 / (self.volatility * np.sqrt(dte / 365))) * (
                            ...
                            + (
                                self.risk_free_rate
                                + (self.volatility * self.volatility) / 2
                            )
                            * (dte / 365)
                        )
            ...
            case "Vega":
                for underlying_price in self.price_range_display:
                    ...
                    for dte in self.time_list_display:
                        ...
                        )  # want to look at change in vol per 1% change in underlying
                        put_row.append(
                            round(
                                underlying_price
                                * norm.pdf(d1)
            ...
            case "Theta":
                for underlying_price in self.price_range_display:
                    ...
                    for dte in self.time_list_display:
                        ...
                        d2 = d1 - self.volatility * np.sqrt(dte / 365)
                        call_theta = (
                            (-underlying_price * norm.pdf(d1) * self.volatility)
                            / (2 * np.sqrt(dte / 365))
                        ) - self.risk_free_rate * self.strike_price * np.exp(
                            -self.risk_free_rate * (dte / 365)
                        ) * norm.cdf(d2)
                        put_theta = (
                            (-underlying_price * norm.pdf(d1) * self.volatility)
                            / (2 * np.sqrt(dte / 365))
                        ...
                        call_row.append(
                            round(call_theta / 365, 3)
                        )  # look at theta per day, hence divide by 365
            ...
            case "Rho":
                for underlying_price in self.price_range_display:
                    ...
                    for dte in self.time_list_display:
                        ...
                        )  # look at changes per 1% change, hence multiply 0.01
                        put_row.append(
                            round(
                                -self.strike_price
                                * (dte / 365)
    ...
    @staticmethod
    def calculate_pnl(
        option_projected_val: List[List[int]], amount_paid: float, denomination: str
    ) -> List[List[int]]:
        pnl = [
            [0.0 for cols in range(0, len(option_projected_val[0]))]
            for rows in range(0, len(option_projected_val))
        ]
