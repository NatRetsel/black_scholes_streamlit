...
st.markdown(
    f'<a href="{linkedin_url}" target="_blank" style="text-decoration: none; color: inherit;"><img src="https://cdn-icons-png.flaticon.com/512/174/174857.png" width="25" height="25" style="vertical-align: middle; margin-right: 10px;">`Lester Tan`</a>',
    unsafe_allow_html=True,
)
...
with col1:
    ...
    current_underlying_price = st.number_input(
        "Current Price of Underlying", value=30.0
    )
...
if time_to_exp_days < 7:
    ...
    time_list_display = [
        time_to_exp_days - (n * (math.ceil(time_to_exp_days / 7))) for n in range(0, 7)
    ]
# print("time to display: ", time_list_display)
b_scholes = BlackScholes(
...
if heatmap_selection == "Greeks":
    ...
    sns.heatmap(
        call_greeks,
        xticklabels=time_list_display,
        yticklabels=np.round(b_scholes.price_range_display, 2),
        annot=True,
        fmt=".2f",
        cmap="RdYlGn",
        ax=ax_call,
...
# else generate PnL heatmaps
    ...
    if heatmap_selection == "Value":
        ...
        sns.heatmap(
            BlackScholes.calculate_pnl(call_value, price_paid, heatmap_selection),
            xticklabels=time_list_display,
            yticklabels=np.round(b_scholes.price_range_display, 2),
            annot=True,
            fmt=".2f",
        ...
        sns.heatmap(
            BlackScholes.calculate_pnl(put_value, price_paid, heatmap_selection),
            xticklabels=time_list_display,
            yticklabels=np.round(b_scholes.price_range_display, 2),
            annot=True,
            fmt=".2f",
